...
logger = logging.getLogger('gentree')
...
def load_graph(source='local', data_dir=None, min_num_nodes=5, max_num_nodes=5000, node_labels=True, graph_labels=True):
  ...
  if source == 'local':
    ...
    return db_graph_loader.load_graph(
      min_num_nodes=min_num_nodes, max_num_nodes=max_num_nodes, node_labels=node_labels, graph_labels=graph_labels
    )