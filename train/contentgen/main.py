...
import sys
import click
from runner import contentgen_runner
from utils import read_config, setup_logger


@click.command()
@click.option('--runner','-r', default='contentgen')
...
def main(runner, mode, data, config_dir, log_level):
  ...
  default_config = os.path.join(config_dir,'contentgen_{}.yaml'.format(data))
  ...
  logger  = setup_logger.setup_logging(log_file, log_level, logger_name='contentgen')
    
  if runner == 'contentgen':
