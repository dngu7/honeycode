...
### Modification of spectral stats for directed grpaphs

def directed_spectral_worker(G):
  #eigs = eigvalsh(nx.normalized_laplacian_matrix(G).todense())  
  eigs = np.linalg.eigvalsh(nx.directed_laplacian_matrix(G).tolist().todense())
  spectral_pmf, _ = np.histogram(eigs, bins=200, range=(-1e-5, 2), density=False)
  spectral_pmf = spectral_pmf / spectral_pmf.sum()
  # from scipy import stats  
  # kernel = stats.gaussian_kde(eigs)
...
def directed_spectral_stats(graph_ref_list, graph_pred_list, is_parallel=True):
  ''' Compute the distance between the degree distributions of two unordered sets of graphs.
    Args:
      graph_ref_list, graph_target_list: two lists of networkx graphs to be evaluated
    '''
  ...
  if is_parallel:
    with concurrent.futures.ThreadPoolExecutor() as executor:
      for spectral_density in executor.map(directed_spectral_worker, graph_ref_list):
        sample_ref.append(spectral_density)
    with concurrent.futures.ThreadPoolExecutor() as executor:
      for spectral_density in executor.map(directed_spectral_worker, graph_pred_list_remove_empty):
        sample_pred.append(spectral_density)
  else:
    for i in range(len(graph_ref_list)):
      spectral_temp = directed_spectral_worker(graph_ref_list[i])
      sample_ref.append(spectral_temp)
    for i in range(len(graph_pred_list_remove_empty)):
      spectral_temp = directed_spectral_worker(graph_pred_list_remove_empty[i])
      sample_pred.append(spectral_temp)
  # print(len(sample_ref), len(sample_pred))
...
def arborescence_worker(G):
  return int(nx.tree.is_arborescence(G))
  

def arborescence_stats(graph_ref_list, graph_pred_list, is_parallel=True):
  sample_ref = []
  sample_pred = []
  graph_pred_list_remove_empty = [
      G for G in graph_pred_list if not G.number_of_nodes() == 0
  ...
  if is_parallel:
    with concurrent.futures.ThreadPoolExecutor() as executor:
      for arbo_stats in executor.map(arborescence_worker, graph_ref_list):
        sample_ref.append(arbo_stats)
    with concurrent.futures.ThreadPoolExecutor() as executor:
      for arbo_stats in executor.map(arborescence_worker, graph_pred_list_remove_empty):
        sample_pred.append(arbo_stats)
  else:
    for i in range(len(graph_ref_list)):
      arbo_stats = arborescence_worker(graph_ref_list[i])
      sample_ref.append(arbo_stats)
    for i in range(len(graph_pred_list_remove_empty)):
      arbo_stats = arborescence_worker(graph_pred_list_remove_empty[i])
      sample_pred.append(arbo_stats)
  
  mmd_arbo = compute_mmd(sample_ref, sample_pred, kernel=gaussian_tv)
  # mmd_dist = compute_mmd(sample_ref, sample_pred, kernel=gaussian)

  if PRINT_TIME:
    print('Time computing arborescence mmd: ', datetime.now() - prev)
  return mmd_arbo
...
def outdegree_worker(G):
  out = [v for k,v in dict(G.out_degree()).items()]
  bins = max(out)
  out_pmf, _ = np.histogram(out, bins=bins, range=(0, bins), density=False)
  out_pmf = out_pmf / out_pmf.sum()
  return out_pmf
  
def outdegree_stats(graph_ref_list, graph_pred_list, is_parallel=True):
  sample_ref = []
  sample_pred = []
  graph_pred_list_remove_empty = [
      G for G in graph_pred_list if not G.number_of_nodes() == 0
  ...
  if is_parallel:
    with concurrent.futures.ThreadPoolExecutor() as executor:
      for outdegree_stats in executor.map(outdegree_worker, graph_ref_list):
        sample_ref.append(outdegree_stats)
    with concurrent.futures.ThreadPoolExecutor() as executor:
      for outdegree_stats in executor.map(outdegree_worker, graph_pred_list_remove_empty):
        sample_pred.append(outdegree_stats)
  else:
    for i in range(len(graph_ref_list)):
      outdegree_stats = outdegree_worker(graph_ref_list[i])
      sample_ref.append(outdegree_stats)
    for i in range(len(graph_pred_list_remove_empty)):
      outdegree_stats = outdegree_worker(graph_pred_list_remove_empty[i])
      sample_pred.append(outdegree_stats)
  
  mmd_outdegree = compute_mmd(sample_ref, sample_pred, kernel=gaussian_tv)
  # mmd_dist = compute_mmd(sample_ref, sample_pred, kernel=gaussian)

  if PRINT_TIME:
    print('Time computing mmd outdegree: ', datetime.now() - prev)
  return mmd_outdegree
...
def tree_stats(graphs):
  ...
  longest_path = [nx.dag_longest_path_length(g) for g in graphs if nx.is_tree(g)]
